...
{
    resolve(@([[self getLiveStreamHelper] canStartLiveStream]));
}
                  ...
                  withRejecter:(RCTPromiseRejectBlock)reject)
{
    dispatch_async(dispatch_get_main_queue(), ^{
        resolve(@([[self getLiveStreamHelper] startLiveStreamWithStreamingURL:streamUrl StreamingKey:streamKey BroadcastURL:broadcastUrl]));
    });
}
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        resolve(@([[self getLiveStreamHelper] stopLiveStream]));
    });
}
