...
{
    resolve(@([[self getRecordingHelper] canStartRecording]));
}
                  ...
                  withRejecter:(RCTPromiseRejectBlock)reject)
{
    dispatch_async(dispatch_get_main_queue(), ^{
           resolve(@([[self getRecordingHelper] startCloudRecording]));
   });
}
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
           resolve(@([[self getRecordingHelper] stopCloudRecording]));
   });
}
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
           resolve(@([[self getRecordingHelper] pauseCloudRecording]));
   });
}
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
           resolve(@([[self getRecordingHelper] resumeCloudRecording]));
   });
}
